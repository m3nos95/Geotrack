...
Related mix / pipe / curb rows from the **same plant** are grouped (e.g. Type C + Type B Superpave, RCP sizes). **GABC, RAP, topsoil, borrow, riprap, and seed stay as their own SECTION** even when they share a plant — each keeps its own SOURCE and ACTION. Tack coat is rewritten as `#401xxx – HMA ITEMS` with a product bullet, matching issued letters. Grouped **pipe / inlet / curb / sidewalk** sizes keep their own SECTION lines (a 24" RCP is not printed as 15"; `#602035` is `MANHOLE, ROUND`, not another 34" inlet). Expansion joint and curing compound listed under `#701` / `#705` print as `#701/705xxx - CONCRETE ITEMS` with a product bullet.
